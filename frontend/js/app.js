...
// ✅ FIX 1: API base URL pointing to your Render backend
const API_BASE = "https://stride-backend-diye.onrender.com";
...
// ✅ FIX 2: apiRequest now prepends API_BASE to all paths
...
// ✅ FIX 3: initAuth now shows a friendly toast if Render is waking up (cold start)
async function initAuth() {
  ...
  try {
    ...
    // Only clear auth on 401 Unauthorized — not on network/timeout errors
    // (Render free tier cold starts can cause transient failures)
    if (e?.status === 401) {
      clearAuth();
    } else {
      console.warn("Auth check failed (server may be waking up):", e?.message);
    }
  }
